...
public class MarcadoresInconsistentesTableModel extends AbstractTableModel {

    private static List<Abastecimento> linhas;
    private static String[] cColunas = new String[]{"Seq", "Data/Hora", "Estação de Abastecimento", "Volume", "Odôm./Horím. Anterior", "Odôm./Horím. Atual", "Veículo"};

    public MarcadoresInconsistentesTableModel() {
        linhas = new ArrayList<>();
    }

    public MarcadoresInconsistentesTableModel(List<Abastecimento> listaAbastecimentos) {
            ...
            case 4:
                if (abastecimento.getVeiculo().getTipo() != null) {
                    if (abastecimento.getVeiculo().getTipo().equals("O")) {
                        return abastecimento.getHodometroAnterior();
                    } else {
                        return abastecimento.getHorimetroAnterior();
                    }
                } else if (abastecimento.getHorimetroAnterior() > 0) {
                    return abastecimento.getHorimetroAnterior();
                } else {
                    return 0L;
                }
